...
func _draw() -> void:
	...
	draw_rect(Rect2(top_left, Vector2(fill_width, bar_height)), Color.BLUE)
	...
	if _preview_active and fill_width > 0.0:
		...
		draw_rect(Rect2(Vector2(overlay_x, top_left.y), Vector2(overlay_width, bar_height)), Color(0.5, 0.9, 1.0, alpha))
